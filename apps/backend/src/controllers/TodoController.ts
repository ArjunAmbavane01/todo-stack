...
export const getTodos = async (req: Request) => {
    try {
        ...
        if (!user) return withCORS(Response.json({ type: "error", message: "Error Fetching User Todos" }, { status: 500 }))

        const todos = user.todos;
        if (todos.length === 0) return withCORS(Response.json({ type: "success", message: "User does not have any todos", todos }, { status: 200 }))

        return withCORS(Response.json({ type: "success", message: "User Todos", todos, }, { status: 200 }))
    } catch (e) {
...
export const getChannelTodos = async (req: Request) => {
    try {
        ...
        return withCORS(Response.json({ type: "success", message: "Channel Todos", todos, }, { status: 200 }))
    } catch (e) {
        return withCORS(Response.json({ type: "error", message: "Internal Server Error" }, { status: 500 }))
    }
}
...
export const addTodo = async (req: Request) => {
    try {
        ...
        if (!title || !description) return withCORS(Response.json({ type: "error", message: "Invalid Todo Body" }, { status: 400 }))

        const todo = await client.todo.create({ data: { title, description, completed: false, userId } })
        return withCORS(Response.json({ type: "success", message: "Todo Added Successfully", todoId: todo.id }, { status: 200 }))
    } catch (e) {
        return withCORS(Response.json({ type: "error", message: "Internal Server Error", }, { status: 500 }))
    }
}
...
export const deleteTodo = async (req: Request) => {
    try {
        ...
        if (!todoId) return withCORS(Response.json({ type: "error", message: "Todo id not present" }, { status: 400 }))

        try {
            await client.todo.delete({ where: { id: todoId, userId } });
            return withCORS(Response.json({ type: "success", message: "Todo Deleted Successfully", }, { status: 200 }));
        } catch (e) {
            return withCORS(Response.json({ type: "error", message: "Todo not found", }, { status: 404 }));
        }
    } catch (e) {
        return withCORS(Response.json({ type: "error", message: "Internal Server Error", }, { status: 500 }))
    }
...
export const updateTodo = async (req: Request) => {
    try {
        ...
        if (!todoId) return withCORS(Response.json({ type: "error", message: "Todo id not present" }, { status: 400 }));
        if (Object.keys(updateBody).length === 0) return withCORS(Response.json({ type: "error", message: "No Valid Keys To update" }, { status: 400 }));

        try {
            await client.todo.update({ where: { id: todoId, userId }, data: updateBody });
            return withCORS(Response.json({ type: "success", message: "Todo Updated Successfully", }, { status: 200 }));
        } catch (e) {
            return withCORS(Response.json({ type: "error", message: "Todo not found"}, { status: 404 }));
        }
    } catch (e) {
